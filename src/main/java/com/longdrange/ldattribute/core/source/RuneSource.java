...
public class RuneSource implements AttributeSource {

    @Override public String getName() { return "符文"; }
    ...
    public List<Entry> getEntries(Player player) {
        ...
        try {
            for (ItemStack card : PlayerData.getCards(player)) {
                ...
                if (cd == null) continue;
                List<String> attrs = com.longdrange.ldattribute.rune.RuneHelper.getCardSocketAttributes(card);
                if (attrs.isEmpty()) continue;
                LDAttributeData d = plugin.getApi().getLoreData(player, null, attrs);
                out.add(new Entry("符文 " + cd.getId(), d));
            }
